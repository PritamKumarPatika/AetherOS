...
def handle_error(error):
    ...
    print(f"[ERR] Unhandled error: {str(error)}")
...
def init_badges():
    try:
        ...
        print(f"[WARN] Badge initialization error: {str(e)}")
...
def seed_data():
    try:
        ...
        user_id = str(user["_id"])  # Extract user_id from user object
        ...
        print(f"[WARN] Seed data error: {str(e)}")
...
if __name__ == "__main__":
    try:
        ...
        print(f"[WARN] Seed data initialization failed: {str(e)}")
    print("[OK] AetherOS Backend running at http://localhost:5000")
